...
> It is NOT the full knowledge base. Use it as the router and `skills/REGISTRY.md` as the index.
...
Do not load the entire `skills/` folder blindly.
Do not treat `CLAUDE.md` as the full knowledge base.
Use `CLAUDE.md` as the router and `skills/REGISTRY.md` as the index.

---

## Baseline Principles (Always Active)

These four principles are always in effect. For the full version, see `skills/shared/karpathy-coding-principles.md`.
...
│
├── memory/                ← Agent memory/session state
├── adr/                   ← Architecture Decision Records
└── reports/               ← Setup and compliance reports
```
...
1. **Always read `skills/REGISTRY.md`** before starting work.
2. **Load `skills/shared/karpathy-coding-principles.md`** as baseline for all coding tasks.
3. **Match task to domain** — load only relevant domain skills.
4. **Skill wrappers point to source** — when a wrapper says "read full skill at `sources/github/.../SKILL.md`", load that file.
...
|---|---|
| Web animations, GSAP, UI effects | `skills/frontend/` |
| APIs, databases, Firebase | `skills/backend/` |
| Security review, pentesting, threats | `skills/security/` |
| Cloud deployment, Kubernetes, CI/CD | `skills/devops/` |
| Unity, game development | `skills/game-dev/` |
| Agent orchestration, RAG, knowledge graphs | `skills/ai-agents/` |
| Planning, TDD, code review, PRDs | `skills/workflow/` |
| Debugging, architecture, general principles | `skills/shared/` |
