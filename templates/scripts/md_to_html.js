...
function convertMdToHtml(mdPath) {
    ...
    let baseDir = path.dirname(path.dirname(absoluteMdPath));
    if (baseDir.includes('reports')) {
        baseDir = path.dirname(baseDir);
    }
    
    // Check if marked is installed in config/
    ...
    try {
        ...
        console.log("The 'marked' package is required. Installing in config/ via npm...");
        try {
            execSync('npm install --prefix config marked', { stdio: 'inherit', cwd: baseDir });
            const markedPath = require.resolve('marked', { paths: [configDir] });
            marked = require(markedPath).marked;
        } catch (installError) {
    ...
    const htmlDir = path.join(baseDir, 'reports', 'html');
